...
export const handleSaveWithImages = async (params) => {
  const data = { ...params.data };

  // Новые изображения
  const newImages = params.data.imagesRaw || [];

  let uploaded = [];
  if (newImages.length) {
    const formData = new FormData();
    newImages.forEach((file) => formData.append('files', file.rawFile));
    const res = await uploadFiles(formData);
    uploaded = res.data.files || [];
  }

  const existing = params.data.images || [];
  const uploadedPaths = uploaded.map((file) => file.filename);
  data.images = [...existing, ...uploadedPaths];

  delete data.imagesRaw;
